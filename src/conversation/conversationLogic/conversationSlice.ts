...
const fetchAnswer = async (question: string) => {
  ...
  // Hardcoded response chunks for testing
  const responseChunks = [
    "This is the first chunk of the response. ",
    "Here comes the second chunk with more text. ",
    "The third chunk continues the streaming response. ",
    "Fourth chunk adds even more content to test the streaming. ",
    "And finally, this is the last chunk to complete the response."
  ];
    ...
    start(controller) {
      ...
      const streamChunks = () => {
        if (currentIndex < responseChunks.length) {
          const chunk = responseChunks[currentIndex];
          console.log(`Streaming chunk ${currentIndex + 1}:`, chunk);
